...
const ValidateAuthToken = () => {
  ...
  useEffect(() => {
    ...
    if (
      !paramsObj.client_info ||
      !paramsObj.code ||
      !paramsObj.session_state ||
      !paramsObj.state
    ) {
      showToastMessage("Invalid parameters: unauthorized access", "error");
      navigate("/login");
    } else {
        handleSubmit(paramsObj);
    }
  ...
  const handleSubmit = async (payload: any) => {
    axiosInstance
      .post("/validate-sso-token", payload)
      .then((response) => {
      ...
      <div className=" pt-8 m-auto flex justify-center flex-col items-center">
        ...
        <div className=" tracking-widest mt-8 text-center">
          <span className="text-gray-500 text-xl">
            Please wait validating the user...
          </span>
        </div>
      </div>
